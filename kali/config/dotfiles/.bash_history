...
git clone https://github.com/stefan-hacks/my-pdrx-config.git .pdrx
z .pdrx
mkdir -p kali/config kali/state
ll
cd stefan-hacks/
ll
cd config/
...
lla
cp .b* ~
cp .inputrc .mostrc .nanorc .vimrc ~
cp .config/* ~/.config/
cp -r .config/* ~/.config/
cd
nvim .bashrc
...
sudo git clone https://github.com/JB63134/bash_ct.git /usr/local/bin/bash_ct
source .bashrc
cd /usr/local/bin
ll
ll  bash_ct/
sudo mv bash_ct/ct.sh .
sudo rm -rf bash_ct/
sudo mv ct.sh ct 
sudo chmod +x ct 
cd
...
eval -- $'git clone https://github.com/YOUR_USERNAME/my-pdrx-config.git ~/.pdrx\ncd ~/.pdrx\npdrx init   # if not yet initialized\npdrx apply\npdrx sync-desktop --restore'
lla
cd
eval -- $'git clone https://github.com/stefan-hacks/pdrx.git\ncd pdrx\nchmod +x pdrx\n./pdrx --install\nsource ~/.bashrc\npdrx init'
./install_manpage.sh 
